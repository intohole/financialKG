...
def validate_relation(relation: Dict[str, Any], entities: List[Dict[str, Any]]) -> bool:
    ...
    try:
        ...
        if not isinstance(relation['head_id'], str) or not isinstance(relation['tail_id'], str):
            return False
        
        # 验证头尾实体是否存在
        entity_ids = {entity['id'] for entity in entities}
        if relation['head_id'] not in entity_ids or relation['tail_id'] not in entity_ids:
            return False
...
def find_entity_by_id(entities: List[Dict[str, Any]], entity_id: str) -> Optional[Dict[str, Any]]:
    """
    通过ID查找实体
...
def deduplicate_entities_by_name(entities: List[Dict[str, Any]], case_sensitive: bool = False) -> List[Dict[str, Any]]:
    """
    根据名称去重实体
...
def find_entity_duplicates(entities: List[Dict[str, Any]], similarity_threshold: float = 0.8) -> List[List[int]]:
    """
    查找实体重复组
    ...
    try:
        ...
        # 筛选有效实体
        valid_entities = [(i, entity) for i, entity in enumerate(entities) if validate_entity(entity) and 'embedding' in entity]
        ...
        for i, (idx1, entity1) in enumerate(valid_entities):
            ...
            for j in range(i + 1, len(valid_entities)):
                ...
                if not visited[idx2]:
                    try:
                        # 计算实体嵌入向量的相似度
                        similarity = calculate_cosine_similarity(entity1['embedding'], entity2['embedding'])
...
def enrich_entity_with_relation_info(entity: Dict[str, Any], relations: List[Dict[str, Any]], all_entities: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    使用关系信息丰富实体数据

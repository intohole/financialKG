...
from .scheduler_config import (SchedulerConfig, SchedulerConfigManager,
                               TaskConfig)
...
class AsyncTaskScheduler:
    ...
    async def start(self) -> None:
        ...
        self.logger.debug(
            f"是否有job_defaults属性: {hasattr(self.scheduler_config, 'job_defaults')}"
        )

        # 创建调度器配置
        jobstores = {"default": self._create_job_store()}

        executors = {"default": self._create_executor()}

        # 确保job_defaults属性存在
        if (
            hasattr(self.scheduler_config, "job_defaults")
            and self.scheduler_config.job_defaults
        ):
            job_defaults = {
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(task_func):
                ...
                result = await loop.run_in_executor(
                    None, lambda: task_func(*task_config.args, **task_config.kwargs)
                )
            ...
            if task_config.retry_count > 0:
                self.logger.warning(
                    f"任务 {task_id} 执行失败，将在 {task_config.retry_delay} 秒后重试"
                )
                ...
                retry_task_config = TaskConfig(
                    ...
                    + timedelta(seconds=task_config.retry_delay),
                )

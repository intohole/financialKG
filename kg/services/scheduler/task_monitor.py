...
from .task_manager import TaskManager, TaskExecutionInfo
...
class TaskMonitor:
    ...
    async def _update_global_metrics(self) -> None:
        ...
        # 计算各种状态的任务数量
        pending_count = sum(1 for status in all_task_status.values() if status == TaskStatus.PENDING)
        running_count = sum(1 for status in all_task_status.values() if status == TaskStatus.RUNNING)
        success_count = sum(1 for status in all_task_status.values() if status == TaskStatus.SUCCESS)
        error_count = sum(1 for status in all_task_status.values() if status == TaskStatus.ERROR)
        missed_count = sum(1 for status in all_task_status.values() if status == TaskStatus.MISSED)
    ...
    def increment_metric(
        ...
        """增加计数器指标"""
        if name in self.global_metrics and self.global_metrics[name].type == MetricType.COUNTER:
    ...
    def get_all_metrics(self) -> Dict[str, Any]:
        ...
        """
        return {
            "global": {name: metric.to_dict() for name, metric in self.global_metrics.items()},
            "tasks": {task_id: metrics.to_dict() for task_id, metrics in self.task_metrics.items()},
            "custom": {name: [m.to_dict() for m in metrics] for name, metrics in self.custom_metrics.items()}
        }
    ...
    def get_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "total_runs": sum(metrics.total_runs for metrics in self.task_metrics.values()),
                "total_successes": sum(metrics.successful_runs for metrics in self.task_metrics.values()),
                "total_failures": sum(metrics.failed_runs for metrics in self.task_metrics.values()),
                "total_missed": sum(metrics.missed_runs for metrics in self.task_metrics.values())
            },
            "top_tasks": {
                "most_runs": [],
    ...
    def _export_prometheus_format(self) -> str:
        ...
        for task_id, metrics in self.task_metrics.items():
            ...
            lines.append(f"# HELP task_{task_name}_total_runs Total runs for task {task_name}")
            lines.append(f"# TYPE task_{task_name}_total_runs counter")
            lines.append(f"task_{task_name}_total_runs {{task_id=\"{task_id}\"}} {metrics.total_runs}")
            
            lines.append(f"# HELP task_{task_name}_successful_runs Successful runs for task {task_name}")
            lines.append(f"# TYPE task_{task_name}_successful_runs counter")
            lines.append(f"task_{task_name}_successful_runs {{task_id=\"{task_id}\"}} {metrics.successful_runs}")
            
            lines.append(f"# HELP task_{task_name}_failed_runs Failed runs for task {task_name}")
            lines.append(f"# TYPE task_{task_name}_failed_runs counter")
            lines.append(f"task_{task_name}_failed_runs {{task_id=\"{task_id}\"}} {metrics.failed_runs}")
            
            lines.append(f"# HELP task_{task_name}_success_rate Success rate for task {task_name}")
            lines.append(f"# TYPE task_{task_name}_success_rate gauge")
            lines.append(f"task_{task_name}_success_rate {{task_id=\"{task_id}\"}} {metrics.success_rate}")
            
            lines.append(f"# HELP task_{task_name}_average_duration Average duration for task {task_name}")
            lines.append(f"# TYPE task_{task_name}_average_duration gauge")
            lines.append(f"task_{task_name}_average_duration {{task_id=\"{task_id}\"}} {metrics.average_duration}")

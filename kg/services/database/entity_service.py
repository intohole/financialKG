...
from kg.database.models import Entity, Relation, News, EntityNews, EntityGroup
from kg.database.repositories import (
    EntityRepository, RelationRepository, NewsRepository, 
    EntityNewsRepository, EntityGroupRepository
)
...
class EntityService:
    ...
    @handle_db_errors_with_reraise()
    async def get_or_create_entity(self, name: str, entity_type: str, canonical_name: Optional[str] = None,
                            properties: Optional[Dict[str, Any]] = None, confidence_score: float = 0.0,
                            source: Optional[str] = None) -> Entity:
        """
        获取或创建实体

...
from kg.database.models import Entity, EntityNews, News
from kg.database.repositories import EntityNewsRepository, NewsRepository
...
class NewsService:
    ...
    def update_extraction_status(self, news_id: int, status: str) -> Optional[News]:
        """更新新闻的提取状态"""
        return self.news_repo.update(
            news_id, extraction_status=status, extracted_at=datetime.utcnow()
        )

    @handle_db_errors_with_reraise()
    async def link_entity_to_news(
        self, entity_id: int, news_id: int, context: Optional[str] = None
    ) -> EntityNews:
        """
        将实体链接到新闻
        ...
        return await self.entity_news_repo.get_or_create(
            entity_id=entity_id, news_id=news_id, context=context
        )

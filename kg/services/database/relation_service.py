...
from kg.database.repositories import (RelationGroupRepository,
                                      RelationRepository)
...
class RelationService:
    ...
    async def find_similar_relations(
        ...
        """
        # 查找相同实体对之间的所有关系
        relations = await self.relation_repo.find_by_entities(
    ...
    async def merge_relations(
        ...
        relation_group = await self.relation_group_repo.create(
            group_name=canonical_relation, description=description
        )
        ...
        logger.info(
            f"合并关系成功: {canonical_relation}, 包含 {len(relation_ids)} 个关系"
        )
    ...
    async def get_relations_by_entity(
        ...
        if as_source:
            source_relations = await self.relation_repo.find_by_source_entity(
                entity_id, relation_type
            )
            relations.extend(source_relations)

        if as_target:
            target_relations = await self.relation_repo.find_by_target_entity(
                entity_id, relation_type
            )
    ...
    async def get_entity_relations(
        ...
        获取两个实体之间的关系
        """
        return await self.relation_repo.find_by_entities(
            source_entity_id, target_entity_id
        )

    @handle_db_errors(default_return=[])
    async def get_all_relations(self, limit: Optional[int] = None, offset: Optional[int] = None) -> List[Relation]:
        """
        获取所有关系
        """
        return await self.relation_repo.get_all(limit=limit, offset=offset)

...
from kg.interfaces.news_processing_service import \
    NewsProcessingServiceInterface
from kg.utils import handle_errors
from kg.utils.service_utils import global_service_registry, register_service
...
class NewsProcessingService(NewsProcessingServiceInterface):
    ...
    async def process_and_store_news(self, news_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        source_url = news_data.get("source_url", news_data.get("url", ""))
        publish_date = news_data.get(
            "publish_date", news_data.get("publish_time", None)
        )
        ...
        if publish_date and isinstance(publish_date, str):
            try:
                ...
                self.logger.warning(
                    f"Invalid publish_date format: {publish_date}, using None instead"
                )
                publish_date = None
        ...
        # 1. 存储新闻基本信息
        news = await self.data_services.create_news(
            title, content, source_url, publish_date, source, author
        )
        ...
        # 3. 存储提取的实体和关系到数据库
        stored_news, stored_entities, stored_relations = (
            await self.data_services.store_llm_extracted_data(
                news.id, entities, relations
            )
        )
        self.logger.info(
            f"提取数据已存储，实体: {len(stored_entities)}, 关系: {len(stored_relations)}"
        )
    ...
    async def get_processing_status(self, news_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "status": status,
                "processed_at": (
                    news.created_at.isoformat() if hasattr(news, "created_at") else None
                ),
        ...
        for i in range(0, total_news, batch_size):
            ...
            if tasks:
                ...
                batch_process_results = (
                    await self._process_tasks_with_concurrency_limit(tasks, batch_size)
                )
            ...
            self.logger.info(
                f"已完成批次处理，进度: {min(i + batch_size, total_news)}/{total_news}"
            )
    ...
    async def _process_tasks_with_concurrency_limit(
        self, tasks: List[asyncio.Task], limit: int
    ) -> List[Any]:
        """
        限制并发数处理任务列表
...
def create_news_processing_service(
    data_services: Any, llm_service: Optional[LLMService] = None
) -> NewsProcessingServiceInterface:
    """
    创建新闻处理服务实例的工厂函数

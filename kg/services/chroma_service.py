...
class ChromaService(VectorDBServiceInterface, AsyncService):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 配置参数
        self.persist_directory = self.config.get("PERSIST_DIRECTORY") or os.getenv(
    ...
    async def _init_client(self):
        ...
        if self.chroma_client is None:
            try:
                ...
                logger.info(
                    f"Chroma客户端已初始化，持久化目录: {self.persist_directory}"
                )
    ...
    async def add_embeddings(
        ...
        logger.info(
            f"成功向集合 {collection_name} 添加了 {len(documents)} 个文档的embedding"
        )
        return True
    ...
    @handle_errors(log_error=True, log_message="初始化Chroma服务失败: {error}")
    async def initialize(self) -> bool:
        ...
        if not self._is_initialized:
            try:
                ...
                logger.error(f"初始化Chroma服务失败: {str(e)}")
                raise
        return True

    @handle_errors(log_error=True, log_message="关闭Chroma服务失败: {error}")

...
if TYPE_CHECKING:
    ...
    from kg.services.embedding_service import \
        ThirdPartyEmbeddingService as EmbeddingService
...
class EntityRelationDeduplicationService(DeduplicationServiceInterface):
    ...
    def __init__(
        ...
        # 依赖注入 - 优先使用传入的服务实例
        self.llm_service = llm_service or self._get_shared_service(
            "llm_service", LLMService
        )
        self.embedding_service = embedding_service or self._get_shared_service(
            "embedding_service", create_embedding_service
        )
        self.chroma_service = chroma_service or self._get_shared_service(
            "chroma_service", self._create_chroma_service
        )
    ...
    async def deduplicate(self, config: DeduplicationConfig) -> DeduplicationResult:
        ...
        try:
            ...
            total_duplicate_relations = relation_result.get(
                "total_duplicate_relations", 0
            )
            ...
            logger.info(
                f"去重操作完成: 处理了 {entities_processed} 个实体, {relations_processed} 个关系, "
    ...
    async def get_deduplication_stats(self) -> Dict[str, Any]:
        ...
        if self._last_deduplication_result:
            ...
            entity_result = self._last_deduplication_result.details.get(
                "entity_deduplication", {}
            )
            relation_result = self._last_deduplication_result.details.get(

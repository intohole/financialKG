...
class LLMService(LLMServiceInterface, AsyncService):
    ...
    async def extract_entities(self, text: str, entity_types: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if entity_types:
            result = await self._entity_extraction.extract_entities_by_type(text, entity_types)
        else:
    ...
    async def extract_relations(self, text: str, entities: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        ...
        if entities:
            result = await self._relation_extraction.extract_relations_with_entities(text, entities)
        else:
    ...
    async def summarize_text(self, text: str, summary_type: str = "standard", **kwargs) -> Dict[str, Any]:
        """
        生成文本摘要
        ...
        if summary_type == "short":
            ...
            result = await self._news_summarization.generate_short_summary(text, max_sentences)
            ...
            result = await self._news_summarization.generate_multi_view_summary(text, perspectives)
        else:
    ...
    async def aggregate_entities(self, entities: List[Dict[str, Any]], aggregation_type: str = "standard", **kwargs) -> Dict[str, Any]:
        ...
        if aggregation_type == "by_type":
            ...
            result = self._entity_aggregation.aggregate_entities_by_type(entities, entity_type)
        elif aggregation_type == "duplicate":
            similarity_threshold = kwargs.get("similarity_threshold", 0.8)
            result = await self._entity_aggregation.find_duplicate_entities(entities, similarity_threshold)
    ...
    async def aggregate_relations(self, relations: List[Dict[str, Any]], aggregation_type: str = "standard", **kwargs) -> Dict[str, Any]:
        ...
        if aggregation_type == "by_type":
            ...
            result = self._relation_aggregation.aggregate_relations_by_type(relations, relation_type)
        elif aggregation_type == "duplicate":
            similarity_threshold = kwargs.get("similarity_threshold", 0.8)
            result = await self._relation_aggregation.find_duplicate_relations(relations, similarity_threshold)
        elif aggregation_type == "merge_attributes":
            result = self._relation_aggregation.merge_relation_attributes(relations)
        elif aggregation_type == "consolidate":
            entities = kwargs.get("entities", [])
            result = self._relation_aggregation.consolidate_relations(relations, entities)
        else:
    ...
    async def process_news_text(self, text: str, options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        entity_aggregation_result = await self.aggregate_entities(entities, entity_aggregation_type, **entity_agg_kwargs)
        ...
        relation_aggregation_result = await self.aggregate_relations(relations, relation_aggregation_type, **relation_agg_kwargs)

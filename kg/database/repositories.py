...
from .connection import db_session, get_db_session
...
class BaseRepository:
    ...
    async def get_all(self, limit: Optional[int] = None, offset: Optional[int] = None, order_by: Optional[str] = None, sort_desc: bool = False) -> Tuple[List[Any], int]:
        """
        获取所有记录并支持分页、排序
        """
        ...
        if order_by:
            ...
            # Default sorting by updated_at descending if column exists
            if hasattr(self.model_class, 'updated_at'):
                stmt = stmt.order_by(desc(self.model_class.updated_at))
        
        # Get total count
        count_stmt = stmt.with_only_columns(func.count(self.model_class.id))
...
class EntityRepository(BaseRepository):
    ...
    async def search_entities(
        ...
        """搜索实体（名称或规范名称包含关键词）并支持分页、排序"""
        # Create the main query
        stmt = select(Entity).where(
            or_(
                Entity.name.like(f"%{keyword}%"),
        ...
        # Get total count
        count_stmt = stmt.with_only_columns(func.count(Entity.id))
        ...
        if self.session and not isinstance(self.session, AsyncIterator):
            ...
            async with db_session() as session:
                ...
                result = await self.session.execute(stmt) if self.session else await session.execute(stmt)
                entities = result.scalars().all()
    ...
    async def get_or_create(self, name: str, entity_type: str, **kwargs) -> Entity:
        ...
        if self.session and not isinstance(self.session, AsyncIterator):
            ...
            if not entity:
                # 处理properties字段，如果是字典则转换为JSON字符串
                if "properties" in kwargs and isinstance(kwargs["properties"], dict):
                    kwargs["properties"] = json.dumps(
                        kwargs["properties"], ensure_ascii=False
                    )
            ...
            async with db_session() as session:
                ...
                if not entity:
                    # 处理properties字段，如果是字典则转换为JSON字符串
                    if "properties" in kwargs and isinstance(
...
class RelationRepository(BaseRepository):
    ...
    async def find_by_source_entity(
        self, source_entity_id: int, relation_type: Optional[str] = None
    ) -> List[Relation]:
        """根据源实体ID查找关系"""
    ...
    async def find_by_target_entity(
        self, target_entity_id: int, relation_type: Optional[str] = None
    ) -> List[Relation]:
        """根据目标实体ID查找关系"""
    ...
    @handle_db_errors(default_return=[])
    async def find_by_type(
        self, relation_type: str, limit: Optional[int] = None
    ) -> List[Relation]:
        """根据关系类型查找关系"""
    ...
    @handle_db_errors(default_return=[])
    async def find_by_canonical_relation(
        self, canonical_relation: str
    ) -> List[Relation]:
        """根据规范关系类型查找关系"""
    ...
    async def get_or_create(
        self, source_entity_id: int, target_entity_id: int, relation_type: str, **kwargs
    ) -> Relation:
        """获取或创建关系"""
        from sqlalchemy import select
        ...
        if self.session and not isinstance(self.session, AsyncIterator):
            ...
            if not relation:
                ...
                await self.session.flush()
                logger.debug(
                    f"创建新关系: {source_entity_id} -> {target_entity_id}, 类型: {relation_type}"
                )
        else:
            # 创建新会话
            async with db_session() as session:
...
class NewsRepository(BaseRepository):
    ...
    @handle_db_errors(default_return=[])
    async def find_by_extraction_status(
    ...
    async def search_news(
        ...
        stmt = select(News).where(
            or_(News.title.like(f"%{keyword}%"), News.content.like(f"%{keyword}%"))
        )
        if limit:
            stmt = stmt.limit(limit)
        result = await self.session.execute(stmt)
    ...
    async def get_recent_news(
        ...
        stmt = (
            select(News)
            .where(News.publish_time >= start_date)
...
class EntityNewsRepository(BaseRepository):
    ...
    @handle_db_errors(default_return=[])
    async def find_by_news(
        self, news_id: int, limit: Optional[int] = None
    ) -> List[EntityNews]:
        """根据新闻ID查找关联"""
    ...
    @handle_db_errors(default_return=None)
    async def find_by_entity_and_news(
        self, entity_id: int, news_id: int
    ) -> Optional[EntityNews]:
        """根据实体ID和新闻ID查找关联"""
    ...
    async def get_or_create(self, entity_id: int, news_id: int, **kwargs) -> EntityNews:
        ...
        if not entity_news:
            ...
            entity_news = EntityNews(entity_id=entity_id, news_id=news_id, **kwargs)
            ...
            if self.session and not isinstance(self.session, AsyncIterator):
                ...
                await self.session.flush()
                logger.debug(
                    f"创建新实体-新闻关联: entity_id={entity_id}, news_id={news_id}"
                )
            else:
                # 创建新会话
                async with db_session() as session:
...
class EntityGroupRepository(BaseRepository):
    ...
    async def find_by_primary_entity(self, primary_entity_id: int) -> List[EntityGroup]:
        ...
        stmt = select(EntityGroup).where(
            EntityGroup.primary_entity_id == primary_entity_id
        )
        result = await self.session.execute(stmt)
        return result.scalars().all()
...
class RelationGroupRepository(BaseRepository):
    ...
    @handle_db_errors_with_reraise()
    async def get_or_create(
        self, group_name: str, parent_id: Optional[int] = None, **kwargs
    ) -> RelationGroup:
        """获取或创建关系分组"""
        stmt = select(RelationGroup).where(RelationGroup.group_name == group_name)
        result = await self.session.execute(stmt)
        ...
        if not relation_group:
            relation_group = RelationGroup(
                group_name=group_name, parent_id=parent_id, **kwargs
            )
    ...
    def get_or_create(self, entity_id: int, news_id: int, **kwargs) -> EntityNews:
        ...
        try:
            ...
            logger.error(
                f"获取或创建实体-新闻关联失败，entity_id: {entity_id}, news_id: {news_id}, 错误: {e}"
            )
            raise
    ...
    async def get_news_by_entity(
        ...
        try:
            stmt = (
                select(News).join(EntityNews).where(EntityNews.entity_id == entity_id)
            )
            if limit:
                stmt = stmt.limit(limit)
            result = await self.session.execute(stmt)
            ...
            logger.error(
                f"根据实体ID获取相关新闻失败，entity_id: {entity_id}, 错误: {e}"
            )
            return []

    async def get_entities_by_news(
        self, news_id: int, limit: Optional[int] = None
    ) -> List[Entity]:
        """根据新闻ID获取相关实体"""
        try:
...
class EntityGroupRepository(BaseRepository):
    ...
    async def find_by_name(self, group_name: str) -> Optional[EntityGroup]:
        ...
        try:
            ...
            logger.error(
                f"根据分组名称查找实体分组失败，group_name: {group_name}, 错误: {e}"
            )
            return None
...
class RelationGroupRepository(BaseRepository):
    ...
    async def find_by_name(self, group_name: str) -> Optional[RelationGroup]:
        ...
        try:
            ...
            logger.error(
                f"根据分组名称查找关系分组失败，group_name: {group_name}, 错误: {e}"
            )
            return None

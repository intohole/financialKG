...
class Entity(Base):
    ...
    # 实体分组ID，用于标识相似实体
    entity_group_id = sa.Column(sa.Integer, sa.ForeignKey('entity_group.id', ondelete='SET NULL'), nullable=True, index=True)
    ...
    __table_args__ = (
        ...
        sa.UniqueConstraint('name', 'type', 'source', name='_entity_name_type_source_uc'),  # 防止重复实体
    )
...
class Relation(Base):
    ...
    # 源实体ID（外键）
    source_entity_id = sa.Column(sa.Integer, sa.ForeignKey('entity.id', ondelete='CASCADE'), nullable=False, index=True)
    
    # 目标实体ID（外键）
    target_entity_id = sa.Column(sa.Integer, sa.ForeignKey('entity.id', ondelete='CASCADE'), nullable=False, index=True)
    ...
    # 关系分组ID，用于标识相似关系
    relation_group_id = sa.Column(sa.Integer, sa.ForeignKey('relation_group.id', ondelete='SET NULL'), nullable=True, index=True)
    ...
    __table_args__ = (
        ...
        Index('idx_relation_target_source', 'target_entity_id', 'source_entity_id'),  # 反向查询索引
        sa.UniqueConstraint('source_entity_id', 'target_entity_id', 'relation_type', 'source', name='_relation_unique_uc'),  # 防止重复关系
    )
...
class EntityNews(Base):
    ...
    # 实体ID（外键）
    entity_id = sa.Column(sa.Integer, sa.ForeignKey('entity.id', ondelete='CASCADE'), nullable=False, index=True)
    
    # 新闻ID（外键）
    news_id = sa.Column(sa.Integer, sa.ForeignKey('news.id', ondelete='CASCADE'), nullable=False, index=True)
...
class EntityGroup(Base):
    ...
    # 主要实体ID（作为该组的代表实体）
    primary_entity_id = sa.Column(sa.Integer, sa.ForeignKey('entity.id', ondelete='SET NULL'), nullable=True)

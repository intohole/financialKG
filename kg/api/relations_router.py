...
from . import schemas
from .deps import get_db, get_knowledge_graph_service, get_entity_relation_deduplication_service
    ...
    def _register_routes(self):
        ...
        async def create_relation(
            ...
            try:
                ...
                raise handle_generic_exception(e, self.logger, "创建关系")
        ...
        async def update_relation(
            ...
            try:
                ...
                from ..utils.exception_handlers import handle_generic_exception
                raise handle_generic_exception(e, self.logger, "更新关系")
        ...
        async def delete_relation(
            ...
            return await self.handle_delete(
                db=db,
                item_id=relation_id,
                service_method=kg_service.delete_relation
            )
        
        @router.post("/deduplicate", response_model=schemas.RelationDeduplicationResponse)
        async def deduplicate_relations(
            request: schemas.RelationDeduplicationRequest,
            deduplication_service: EntityRelationDeduplicationService = Depends(get_entity_relation_deduplication_service)
        ):
            """关系去重端点"""
            try:
                self.logger.debug(f"关系去重请求: {request.dict()}")
                ...
                if request.entity_id is not None:
                    ...
                    self.logger.debug(f"按关系类型去重: relation_type={request.relation_type}")
                ...
                self.logger.info(f"关系去重完成，处理关系数量: {result.get('processed_count', 0)}")
                return schemas.RelationDeduplicationResponse(**result)
            except ValueError as ve:
                self.logger.error(f"参数验证错误: {str(ve)}")
                raise HTTPException(status_code=400, detail=str(ve))
            except Exception as e:
                self.logger.exception(f"关系去重处理失败: {str(e)}")
                raise HTTPException(status_code=500, detail=f"关系去重处理失败: {str(e)}")


# 创建路由实例
relation_router_instance = RelationRouter()
relations_router = relation_router_instance.get_router()
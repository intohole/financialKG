...
class BaseConfig:
        ...
        try:
            ...
            return {
                ...
                "scheduler": {
                    ...
                    "job_store": {"type": "memory", "url": None},
                    "executor": {"type": "asyncio", "max_workers": 10},
                    "tasks": [],
                },
            }
    ...
    def base_url(self) -> Optional[str]:
        ...
        获取API基础URL
        """
        return self.config.get("llm", {}).get("base_url") or os.getenv(
            "OPENAI_BASE_URL"
        )
...
class LLMConfig(BaseConfig):
    ...
    def _init_client(self):
        ...
        if not self.api_key:
            raise ValueError(
                "未找到OpenAI API密钥，请在配置文件中设置或通过环境变量OPENAI_API_KEY提供"
            )
...
class EmbeddingConfig(BaseConfig):
    ...
    def _init_client(self):
        ...
        if not api_key:
            raise ValueError(
                "未找到嵌入API密钥，请在配置文件中设置或通过环境变量EMBEDDING_API_KEY提供"
            )
...
class TaskConfig:
    ...
    def __init__(
        ...
        task_trigger: Dict[str, Any] = None,
...
class SchedulerConfigManager:
    ...
    def remove_task(self, task_id: str) -> bool:
        ...
        scheduler_config.tasks = [
            task for task in scheduler_config.tasks if task.task_id != task_id
        ]
...
class SchedulerConfigWrapper(BaseConfig):
    ...
    def log_format(self) -> str:
        return self.config.get("scheduler", {}).get(
            "log_format", "%(asctime)s - %(name)s - %(levelname)s - %(message)s"
        )
...
class DeduplicationConfig(BaseConfig):
    ...
    def relation_weight_threshold(self) -> float:
        """关系权重阈值"""
        return self.config.get("deduplication", {}).get(
